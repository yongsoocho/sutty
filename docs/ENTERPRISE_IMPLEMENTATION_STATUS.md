...
| Interactive terminal | Persistent PTY, server-side resize, bounded output handling, UTF-8 decoding, control/navigation/function keys, and a bounded transitional VT renderer. |
...
2. Complete OTP and multi-prompt keyboard-interactive authentication, Windows SSH agent support, jump hosts, proxies, reconnect policy, and algorithm-policy UX.
