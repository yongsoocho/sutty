...
| Multi SFTP | Explicitly checked SFTP sessions receive one file/folder independently, with per-server progress/result isolation and failed-server-only retry. |
...
| Route foundation | Direct, HTTP CONNECT, SOCKS4, SOCKS5, SSH jump, and ProxyCommand routes are resolved before client creation and shared by SSH/SFTP. Enterprise mode rejects Direct without fallback; one credential-free correlation context is created per session. |
...
3. Complete collision and permission policy, pause, recursive delete, restart job discovery, and large-file/deep-path/multi-host SFTP evidence.
