...
| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| SFTP-001 | P0 | Partial | Dual-pane Files provides absolute-path entry, parent navigation, refresh, and extended multi-selection for a read-only local Details list and the active session's remote current-folder list. Remote lazy loading, bounded flattened enumeration, and bounded recursive filename search remain available; symlinks are listed but not traversed. Sorting controls, favorite paths, synchronized browsing, broad permission/error handling, and live-server search acceptance are missing.<br>Dual-pane Files는 읽기 전용 Local Details 목록과 활성 세션의 Remote 현재 폴더 목록에 절대 경로 입력·상위 폴더·새로 고침·다중 선택을 제공합니다. 원격 지연 로딩·제한된 평면 열거·제한된 재귀 파일명 검색을 유지하고 symlink는 표시하되 따라가지 않습니다. 정렬 컨트롤·즐겨찾는 경로·동기 탐색·폭넓은 권한/오류 처리와 실서버 검색 인수는 남아 있습니다. |
| SFTP-002 | P0 | Partial | Explicit Local → Remote and Remote → Local buttons queue multiple selected files or recursive directories, including empty folders, through the existing collision, staging, checkpoint, and verification path. In-app pane-to-pane drag-and-drop and 100GB/full Unicode/deep-path/live-server acceptance are missing; Explorer → Remote drop remains available.<br>명시적 Local → Remote 및 Remote → Local 버튼이 선택한 여러 파일과 빈 폴더를 포함한 재귀 디렉터리를 기존 충돌·staging·checkpoint·검증 경로로 전송 큐에 넣습니다. 앱 내부 pane 간 drag-and-drop과 100GB·전체 Unicode·깊은 경로·실서버 인수는 남아 있고 Explorer → Remote drop은 유지합니다. |
