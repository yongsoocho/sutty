...
This document maps the product specification and the September 2026 daily-workflow additions to the current Sutty working tree. It is a development trace, not a GA certification. See [Daily workflow](DAILY_WORKFLOW.md) for usage and short reproduction steps.

이 문서는 제품 명세와 2026년 9월 일상 작업 개선을 현재 Sutty 작업 트리에 연결합니다. 개발 추적표이며 GA 인증이 아닙니다. 사용법과 짧은 재현 절차는 [일상 작업 안내](DAILY_WORKFLOW.md)에 있습니다.
...
| UX-A11Y-002 | P1 | Partial | [36 named app/terminal palettes](THEMES.md) retain gradient accents; following the app uses its named ANSI palette. Cards, toolbars, Files panes, and supporting panes reflow with available width; Settings fills the workspace while sessions stay alive. High Contrast, 200% text-scale, and visual acceptance remain unverified.<br>[이름 있는 앱·터미널 팔레트 36개](THEMES.md)가 그라데이션 강조를 유지하며 앱 테마에 맞춤은 같은 이름의 ANSI 팔레트를 사용합니다. 카드·도구 모음·Files·보조 패널은 너비에 맞춰 재배치하고 Settings는 세션을 유지하며 작업 영역을 채웁니다. High Contrast·200% 텍스트 확대·화면 인수는 미검증입니다. |
...
| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| HOST-001 | P0 | Partial | Saved Hosts support create/update, credential-free duplicate, delete, groups, environments, tags, favorites, authentication aliases, and existing format imports. Import previews additions/changes/duplicates/errors with per-row Add/Skip/Copy/Update choices. Broader bulk management and manual cross-PC acceptance remain.<br>저장 호스트는 생성·수정·자격증명 없는 복제·삭제·그룹·환경·태그·즐겨찾기·인증 별칭과 기존 형식 가져오기를 지원합니다. 추가·변경·중복·오류를 미리 보고 항목별 추가/건너뛰기/복제/갱신을 선택합니다. 폭넓은 일괄 관리와 실제 PC 간 인수는 남아 있습니다. |
...
| HOST-004 | P0 | Partial | SSH, Terminal, and SFTP state remain independent; a runtime Tunnels page now shows bind/destination, listener state/errors, start/stop, and session-only additions. Actual server failure/isolation acceptance remains.<br>SSH·Terminal·SFTP 상태를 분리하며 실행 중 Tunnels 화면에서 바인드·목적지·수신 상태·오류·시작/중지·이번 세션 규칙 추가를 제공합니다. 실제 서버의 실패·격리 인수는 남아 있습니다. |
| HOST-005 | P1 | Implemented | A credential-free atomic workspace snapshot remembers up to 16 local tabs with their PowerShell/CMD choice and Saved Host ids. Missing or unknown shell values restore PowerShell. Startup restoration is optional, reconnect confirmation is enabled by default, missing profiles are skipped, and terminal commands are never stored or replayed.<br>비밀정보 없는 원자적 Workspace snapshot이 최대 16개의 로컬 탭별 PowerShell/CMD 선택과 저장 Host ID를 기억합니다. 셸 정보가 없거나 알 수 없으면 PowerShell로 복원합니다. 시작 복원은 선택형이고 재연결 확인이 기본 활성화되며, 사라진 프로필은 건너뛰고 터미널 명령은 저장하거나 재실행하지 않습니다. |
...
| SSH-008 | P1 | Partial | Keepalive is applied per connection. Failed or disconnected sessions offer a manual reconnect that always creates a fresh shell: Saved Hosts reload the current profile and optional vault secret, while one-off sessions return a credential-free draft to Quick Connect. Commands, terminal input, prompt delegates, trust-once decisions, and transport objects are never replayed. Opt-in automatic reconnect/backoff and explicit SFTP/tunnel recovery policies are missing, and live reconnect acceptance remains.<br>연결별 keepalive를 적용합니다. 실패하거나 끊긴 세션은 항상 새 Shell을 만드는 수동 재연결을 제공하며, 저장 Host는 현재 profile과 선택형 Vault 비밀값을 다시 읽고 일회성 세션은 비밀값 없는 초안을 Quick Connect로 돌려보냅니다. 명령·터미널 입력·prompt delegate·이번만 신뢰 결정·transport 객체는 재실행하거나 재사용하지 않습니다. 선택형 자동 재연결·backoff와 명시적 SFTP·tunnel 복구 정책은 없으며 실제 재연결 인수도 남아 있습니다. |
...
The workspace centers on the selected local or SSH terminal. Fresh startup and closing the last tab open PowerShell; `+` / `Ctrl+T` also offers CMD. Home/Hosts/Transfers/Commands open on the right in wide windows and below the shell under 1100 logical pixels. Settings covers the shell as a full page while sessions remain alive. SSH Files/Commands/Tunnels move below the terminal in narrow layouts, and cards/file panes reflow. Windows UI automation on 2026-09-22 was **stopped by the user's Escape input**; visual acceptance remains unverified. The package-local renderer is integrated, but GA requires the remaining compatibility and security matrix. See [ADR 0001](adr/0001-terminal-renderer.md).

선택한 로컬 또는 SSH 터미널을 중심으로 작업합니다. 새로 시작하거나 마지막 탭을 닫으면 PowerShell을 열고, `+` / `Ctrl+T`에서 CMD도 선택할 수 있습니다. Home/Hosts/Transfers/Commands는 넓은 창의 오른쪽이나 논리 픽셀 1100 미만 창의 아래에 엽니다. Settings는 전체 페이지로 셸을 가리며 세션은 계속 실행됩니다. SSH Files/Commands/Tunnels는 좁은 화면에서 터미널 아래로 이동하고 카드·파일 패널도 재배치합니다. **2026-09-22 Windows 화면 자동화는 사용자의 Escape 입력으로 중단**되어 화면 인수는 미검증입니다. 패키지 내부 렌더러는 통합됐지만 GA에는 남은 호환성·보안 매트릭스가 필요합니다. [ADR 0001](adr/0001-terminal-renderer.md)을 확인하세요.

| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| TERM-001 | P0 | Partial | Persistent SSH `ShellStream` PTY and local PowerShell/CMD ConPTY use the package-local xterm.js renderer. The required vim/tmux/htop/sudo compatibility matrix and manual CMD input/resize acceptance remain unverified.<br>지속 SSH `ShellStream` PTY와 로컬 PowerShell/CMD ConPTY가 패키지 내부 xterm.js 렌더러를 사용합니다. 필수 vim/tmux/htop/sudo 호환 매트릭스와 CMD 입력·크기 변경 수동 인수는 아직 미검증입니다. |
...
| TERM-006 | P1 | Partial | Bounded configurable scrollback, selection, Ctrl+Insert copy, Shift+Insert paste, bracketed-paste-aware input, and Ctrl+F search exist. A clipboard icon beside CONPTY/PTY dimensions copies the latest rendered command output, including errors. PowerShell/CMD use runtime prompt markers without profile-file edits; custom/unmarked SSH command boundaries are best effort. PTY tabs, control sequences, and newlines are already rendered, so this is not original stdout/stderr bytes. Manual clipboard, 100,000-line, latency, and long-session acceptance remain.<br>제한·설정 가능한 scrollback, 선택, Ctrl+Insert 복사, Shift+Insert 붙여넣기, bracketed paste 입력, Ctrl+F 검색을 지원합니다. CONPTY/PTY 크기 옆의 클립보드 아이콘은 오류를 포함한 마지막 명령의 표시 출력을 복사합니다. PowerShell/CMD는 프로필 파일 수정 없이 실행 중 프롬프트 표시자를 사용하며 사용자 정의·표시자 없는 SSH 명령 경계는 최선 방식으로 판별합니다. 탭·제어 문자·줄바꿈을 PTY가 처리한 텍스트이므로 원본 stdout/stderr 바이트가 아닙니다. 수동 클립보드·100,000줄·지연·장시간 세션 인수는 남아 있습니다. |
...
| TERM-009 | P1 | Partial | Home validates and launches one-line direct connection commands through local ConPTY as an executable plus argument vector, without `cmd.exe`, PowerShell, or shell expansion. `ssh worker1` uses the installed OpenSSH `.ssh/config`, `IdentityFile`, SSH Agent, and `PATH`; `multipass connect master` runs the installed Multipass executable. Favorites (100 maximum) and bounded local history (250 maximum) retain no resolved executable path, terminal output, password, token, or passphrase, and reject credential-like storage. Focused planner/store tests exist; actual SSH Agent/key/config, Multipass, and UI acceptance remain unverified.<br>Home은 한 줄 직접 연결 명령을 검증해 `cmd.exe`·PowerShell·셸 확장 없이 실행 파일과 인수 벡터로 로컬 ConPTY에 실행합니다. `ssh worker1`은 설치된 OpenSSH의 `.ssh/config`·`IdentityFile`·SSH Agent·`PATH`를 사용하고, `multipass connect master`는 설치된 Multipass 실행 파일을 실행합니다. 즐겨찾기(최대 100개)와 제한된 로컬 기록(최대 250개)은 해석된 실행 파일 경로·터미널 출력·비밀번호·토큰·passphrase를 보관하지 않고 자격증명으로 보이는 값의 저장을 거부합니다. planner/store 집중 테스트는 있지만 실제 SSH Agent·키/config·Multipass·UI 인수는 아직 미검증입니다. |

## SFTP file system / SFTP 파일 시스템

| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| SFTP-001 | P0 | Partial | Both panes offer absolute paths, parent/refresh, bounded successful-navigation back/forward history, hidden toggles, folder-first name/size/modified sorting, and multi-selection. Remote favorites persist separately per host; malformed/future favorite files are preserved on read failure. Bounded remote search and non-traversed symlinks remain. Live server/permission/search and manual UI acceptance remain unverified; synchronized browsing is separate.<br>양쪽 패널에 절대 경로·상위/새로 고침·성공한 탐색만 기록하는 제한된 뒤로/앞으로·숨김 표시·폴더 우선 이름/크기/수정일 정렬·다중 선택을 제공합니다. 원격 즐겨찾기는 호스트별 저장하며 손상·미지원 버전 파일은 읽기 실패 시 보존합니다. 제한된 원격 검색과 symlink 비순회도 유지합니다. 실제 서버·권한·검색·수동 UI 인수는 미검증이며 동기 탐색은 별도입니다. |
| SFTP-002 | P0 | Partial | Pane-to-pane file/folder drag-and-drop, explicit transfer buttons, and Explorer → Remote drop use the existing durable collision/staging/checkpoint/verification path, including recursive and empty directories. Drops copy without source removal and pin the session/destination before dialogs. Manual drag acceptance and full Unicode/deep-path/live-server scale evidence remain.<br>패널 간 파일·폴더 드래그앤드롭, 명시적 전송 버튼, Explorer → Remote 드롭은 재귀·빈 폴더를 포함해 기존 영속 충돌/staging/checkpoint/검증 경로를 사용합니다. 드롭은 원본을 삭제하지 않는 복사이며 대화상자 전에 세션·목적지를 고정합니다. 수동 드래그 인수와 전체 Unicode·깊은 경로·실서버 규모 증거는 남아 있습니다. |
...
## File editing and terminal linkage / 파일 편집과 터미널 연계

| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| EDIT-001 | P0 | Partial | Regular text files up to 8 MiB open in a configured external executable. Save detection defaults to manual upload, with explicit per-file automatic upload. Host/path, immutable upload source, remote size/time review, and the existing safe durable queue are used; failed jobs require Edits review. Reload commits only after verification, retaining previous copies on failure; close warns and keeps recovery files. Metadata cannot prevent every concurrent edit, and editor buffers cannot be inspected. Manual save/conflict/failure/close and real server acceptance remain unverified.<br>8 MiB 이하 일반 텍스트 파일을 지정한 외부 실행 파일로 엽니다. 저장 감지 후 수동 반영이 기본이며 파일별 자동 반영을 명시적으로 켤 수 있습니다. 고정 호스트/경로·업로드 사본·원격 크기/수정시각 검토·기존 안전 영속 큐를 사용하고 실패 작업은 편집본에서 다시 확인합니다. 재다운로드는 검증 후 교체하여 실패 시 기존 편집본을 보존하고, 종료 경고 후 복구 파일을 남깁니다. 메타데이터로 모든 동시 수정을 막거나 편집기 버퍼를 확인할 수는 없습니다. 수동 저장·충돌·실패·종료와 실서버 인수는 미검증입니다. |
| LINK-001 | P0 | Partial | Open in terminal previews and copies a safely quoted POSIX directory command without a newline, never injecting or executing it. Terminal → Files uses explicit absolute-path input. Quoting/control-character tests exist; manual shell/TUI acceptance remains. Automatic output parsing or directory tracking is absent.<br>터미널에서 열기는 안전하게 인용한 POSIX 경로 이동 명령을 미리 보여주고 줄바꿈 없이 복사하며 직접 입력·실행하지 않습니다. Terminal → Files는 명시적 절대 경로 입력을 사용합니다. 인용·제어 문자 테스트가 있고 수동 셸/TUI 인수는 남아 있습니다. 출력 자동 파싱이나 경로 추적은 없습니다. |

## Transfer manager / 전송 관리자

| ID | Priority | Status | Evidence and remaining gap / 증거와 남은 차이 |
| --- | --- | --- | --- |
| XFER-001 | P0 | Partial | A credential-free atomic job store, per-panel bounded workers, and a live global Transfer Center exist. The global surface filters state/direction/target, removes completed records, and enables commands only when the exact connected single-transfer executor can accept them. Queue mutations and target execution are serialized across Sutty processes. Configurable global/per-host concurrency and global controls for Multi batches remain incomplete.<br>자격증명 없는 atomic job 저장소·패널별 제한 worker·실시간 전역 Transfer Center가 있습니다. 전역 화면은 상태·방향·대상을 필터링하고 완료 기록을 제거하며 정확히 일치하는 연결된 단일 전송 실행자가 받을 수 있을 때만 명령을 활성화합니다. 큐 변경과 대상 실행은 여러 Sutty 프로세스 사이에서 직렬화합니다. 설정 가능한 전역·Host별 동시성과 Multi batch 전역 제어는 미완성입니다. |
| XFER-002 | P0 | Partial | Progress, speed, ETA, direction, and state are visible. The global projection coalesces in-process queue events and external queue-file changes before refreshing; large-file refresh and performance evidence is missing.<br>진행률·속도·ETA·방향·상태를 표시합니다. 전역 투영은 같은 프로세스의 큐 event와 외부 큐 파일 변경을 합쳐 갱신하며 대용량 파일 갱신·성능 증거는 남아 있습니다. |
| XFER-003 | P0 | Partial | Queued/running cancellation, durable pause/resume that preserves checkpoints, temporary-file retention/cleanup, and configurable transient retry exist. Global Pause/Resume/failed-only Retry/Cancel routes to an exact connected Files executor and otherwise fails closed. Multi batch-wide global Pause/Cancel, stalled-call interruption beyond transport cancellation, and live failure evidence remain.<br>대기·실행 취소, checkpoint를 보존하는 영속 일시정지·재개, 임시 파일 유지·정리, 설정 가능한 일시 오류 재시도가 있습니다. 전역 일시정지·재개·실패 대상만 재시도·취소는 정확히 일치하는 연결된 Files 실행자에 전달하고 그 외에는 기본 차단합니다. Multi batch 전체의 전역 일시정지·취소, transport 취소를 넘는 stalled-call 중단과 실환경 실패 증거는 남아 있습니다. |
...
| XFER-006 | P1 | Implemented | Non-secret checkpoints and job/target state survive restart. Files and Multi discover matching incomplete work and require an explicit resume action; abandoned running work becomes interrupted and successful targets stay complete. Operating-system file leases prevent the same target from executing concurrently across Sutty processes, and a document lock prevents concurrent writers from losing queue jobs.<br>비밀정보 없는 checkpoint와 job·대상 상태가 재시작 뒤 유지됩니다. Files·Multi가 일치하는 미완료 작업을 찾아 명시적 재개를 요구하며 중단된 실행 작업은 interrupted로 바뀌고 성공 대상은 완료 상태를 유지합니다. 운영체제 파일 lease가 여러 Sutty 프로세스에서 같은 대상의 동시 실행을 막고 document lock이 동시 writer의 큐 작업 유실을 방지합니다. |
...
| XFER-008 | P1 | Partial | Explicitly checked sessions support 1→N upload and N→1 download with preflight target/source/destination/policy review, independent progress/result state, deterministic destination isolation, and failed/incomplete-only retry. New, retry, and restored execution acquires one cross-process lease per target for the full batch. Batch-wide global Pause/Cancel and live 16-target/cancellation evidence remain.<br>명시적으로 체크한 세션은 실행 전 대상·원본·대상 경로·충돌 정책 검토와 함께 1→N 업로드·N→1 다운로드, 독립 진행률·결과, 결정적 목적지 분리, 실패·미완료 대상만 재시도를 지원합니다. 신규·재시도·복원 실행은 batch 전체 동안 대상별 cross-process lease를 얻습니다. Batch 전체 전역 일시정지·취소와 실제 16대·취소 검증은 남아 있습니다. |
...
| TUN-004 | P2 | Partial | A post-connect Tunnels page adds stopped session-only local/remote/dynamic rules and starts/stops them with state, bind/destination, and actionable errors. Loopback is the default and external starts require confirmation. Focused lifecycle/failure tests exist; actual forwarding, port conflicts, server policy, and close acceptance remain.<br>연결 후 Tunnels에서 Local·Remote·Dynamic 규칙을 이번 세션의 중지 상태로 추가하고, 상태·바인드/목적지·조치 가능한 오류를 보며 시작/중지할 수 있습니다. loopback이 기본이며 외부 수신 시작은 확인합니다. 수명주기·실패 집중 테스트가 있고 실제 포워딩·포트 충돌·서버 정책·종료 인수는 남아 있습니다. |
...
| IMP-004 | P1 | Partial | Reviewed schemaVersion 1 JSON shares selected host/group/tag/route/tunnel/command definitions (4 MiB / 1,000 items) with authentication aliases. Import validates schema and previews per-row Add/Skip/Copy/Update and unsupported items; newer versions are refused. Credential values/references, private-key paths, trust, and histories are excluded. ProxyCommand content is omitted and imported external-command routes remain blocked; commands never run on import. User-authored command secrets and server identifiers require review. Cross-PC credential binding and manual acceptance remain unverified.<br>검토한 schemaVersion 1 JSON으로 인증 별칭을 포함한 선택 Host/그룹/태그/route/tunnel/명령 정의를 공유합니다(4 MiB/1,000개). 가져오기는 schema 검사 후 항목별 추가/건너뛰기/복제/갱신과 미지원 항목을 미리 보며 새 버전을 거부합니다. 자격증명 값/참조·개인키 경로·신뢰·기록은 제외합니다. ProxyCommand 내용은 빼고 가져온 외부 명령 경로는 차단하며 명령을 자동 실행하지 않습니다. 사용자가 명령에 쓴 비밀값과 서버 식별 정보는 검토해야 합니다. PC 간 자격증명 연결과 수동 인수는 미검증입니다. |
...
| EVID-001 | P0 | Implemented | [Evidence schema](evidence/EVIDENCE_SCHEMA.md) defines the exact manifest/summary/review contract, root allowlist, post-run write-once human-review transformation, append-only Git-history guard, and release-attestation mapping with fixture-validated rejection boundaries. It creates no live `Pass` record; a reviewed real bundle is still required for every live claim.<br>[증거 스키마](evidence/EVIDENCE_SCHEMA.md)는 정확한 manifest/summary/review 계약, root allowlist, 실행 후 write-once 사람 검토 변환, append-only Git history guard, release-attestation 연결과 fixture 기반 거부 경계를 정의합니다. 실환경 `Pass` 기록을 만들지 않으며 모든 실환경 주장에는 검토 완료한 실제 bundle이 필요합니다. |
...
| NFR-009 | Supportability / 지원성 | Partial | Connection Doctor classifies nine stages with stable codes, user actions, safe copy summaries, and correlation-scoped structured events. Settings selects an exact open connection or retained endpoint-free failure, previews its local-only identity and bounded diagnostics, and binds ZIP creation to the previewed event-snapshot digest. Captured failures are authoritative; a validated stage-aware context code recovers a missing failure append, an explicit same-stage success prevents stale fallback, and conflicting non-`NONE` codes fail closed. The two-entry ZIP excludes session labels, endpoints, users, paths, fingerprints, transcripts, commands, output, secrets, and raw logs. Detailed local logs carry a persistent infrastructure-identifier warning and an explicit raw-copy label; real support workflow and accessibility acceptance remain.<br>Connection Doctor는 9단계를 안정적 code·사용자 조치·안전한 복사 요약·correlation별 구조화 event로 분류합니다. 설정에서 정확한 열린 연결 또는 Endpoint 없는 보존 실패를 선택하고 로컬 전용 식별 정보와 제한된 진단을 미리 본 뒤, 해당 event snapshot digest에 ZIP 생성을 결속합니다. 캡처된 실패를 우선하며, 검증된 단계별 Context code는 누락된 실패 event를 복구하고 같은 단계의 명시적 성공은 오래된 fallback을 막으며 서로 다른 non-`NONE` code는 기본 차단합니다. 두 Entry ZIP은 세션 이름·Endpoint·사용자·경로·fingerprint·transcript·command·output·secret·raw log를 제외합니다. 로컬 상세 로그에는 인프라 식별 정보 경고와 명시적인 원시 복사 표시가 있으며 실제 지원 흐름과 접근성 인수는 남아 있습니다. |
