...
앱을 새로 시작하면 중앙에 PowerShell이 열립니다. 상단 `+` 또는 `Ctrl+T`에서 **로컬 PowerShell**이나 **로컬 CMD**를 선택해 셸 탭을 추가합니다. 마지막 탭을 수동으로 닫으면 새 PowerShell 탭을 열어 중앙을 셸 화면으로 유지합니다. 작업 공간 복원을 켜면 로컬 탭별 PowerShell/CMD 선택을 복원하며, 셸 정보가 없는 예전 항목은 PowerShell로 엽니다.
...
**Multi Command**는 왼쪽 메뉴나 `Alt+8`에서 독립적으로 엽니다. 이 화면에서는 중앙 셸을 가리고 **3×3 세션 그리드**를 표시하며, 열린 셸은 계속 실행됩니다. 각 칸에 열린 SSH·PowerShell·CMD 세션 하나를 표시하고, 최대 16개 열린 탭 중 첫 9개를 표시합니다. 이전/다음 페이지 UI는 숨기며 나머지 탭은 제외 개수만 안내하고 실행하지 않습니다. 좁은 화면에서도 세 열을 유지하며 스크롤할 수 있습니다.

오른쪽은 기존 **명령 모음집**을 재사용하므로 검색·추가·삭제·`$1`, `$2` 값 입력을 그대로 사용할 수 있습니다. 위쪽에 직접 입력한 명령과 저장 명령 모두 **체크한 세션에만** 실행합니다. Sutty SSH는 구조화된 exec를 사용합니다. 로컬·외부 탭은 매 실행마다 기본 해제된 고급 입력 확인을 별도로 승인해야 합니다. 이 입력은 현재 전경 프로그램에 전송되므로 편집기·인증 대기 화면에서도 명령으로 해석된다고 보장하지 않습니다. 종료 코드를 알 수 없으면 완료·종료 코드 미확인으로 표시합니다. 실행 전 전체 대상 수·정확한 대상과 명령을 확인하며 취소해도 초안을 유지합니다. SSH exec는 현재 터미널의 cd·환경변수·sudo 상태를 공유하지 않습니다. 60초 대기 제한과 대기 취소를 제공하며 원격 작업의 종료를 보장하지 않습니다. **전체 선택**은 보이는 실행 중인 로컬·외부 탭과 연결된 Sutty SSH를 선택하며 **전체 해제**는 선택 상태를 지웁니다. 숨겨진 탭과 미연결 탭은 실행 대상에서 제외하고 개수를 표시합니다. 새로 연 세션은 항상 미선택이고 기본 대상은 0개입니다. 셸 탭·메뉴를 바꾸어도 같은 열린 세션의 선택·마지막 결과·진행 상태를 유지합니다. 각 칸에서 세션 상태와 마지막 결과를 확인하고 최대 16,384자의 출력 미리보기를 스크롤해 읽을 수 있습니다. PROD 태그가 있는 대상은 추가 확인을 거칩니다.

## 접속과 파일 탐색

Transfers나 `Alt+7`의 파일 탐색은 현재 선택한 셸을 따릅니다. SSH에서는 같은 세션의 Local/Remote Files를, PowerShell/CMD에서는 프롬프트가 알린 현재 작업 폴더를 표시합니다. 다른 서버를 대신 선택하지 않으며 외부 탭이나 선택한 셸이 없으면 이 PC의 로컬 홈 폴더를 표시하고 별도 SSH 연결을 안내합니다. 로컬 탐색기의 폴더 이동은 셸의 디렉터리를 바꾸지 않고 **현재 셸 폴더**로 다시 따라갈 수 있습니다. 직접 실행한 SSH/Multipass는 터미널 전용이며 원격 Files·Tunnels를 통합하지 않습니다. 호스트 가져오기 또는 저장 호스트 연결로 통합 도구를 사용하세요.
...
셸 사용 후 로그아웃·연결 종료 시 기본적으로 해당 탭만 자동으로 닫습니다. 설정에서 자동 닫기를 끄면 종료된 탭을 유지합니다. 마지막 탭의 자동 종료는 Home으로 돌아가며 셸을 다시 실행하지 않습니다. 최초 연결 실패는 진단하도록 남겨두고, 전송·편집 복구가 필요하면 기존 확인창을 유지합니다. 입력한 logout 문자열이 아닌 실제 PTY·프로세스·연결 상태를 사용합니다.

A 서버 터미널에서 `ssh B`를 실행해도 Files는 A에 연결된 상태입니다. B 파일을 보려면 새 연결의 목적지를 B로 입력하고 **고급 연결 옵션 → 연결 경로 → SSH Jump**에서 A를 경유 서버로 지정하세요. 그러면 Terminal과 Files가 모두 B에 연결됩니다. A가 TCP forwarding과 B 접근을 허용하고 B가 SFTP를 제공해야 합니다. 목적지 인증은 이 PC에서 사용할 수 있어야 하며 A에만 있는 키는 자동 재사용하지 않습니다. Files 상단에서 실제 SFTP 대상을 확인할 수 있습니다.

## 한 줄 로컬 연결

**한 줄 연결**은 Home의 새 연결 아래 독립 카드입니다. 저장한 명령은 **Hosts 즐겨찾기**에 나타납니다. 같은 명령을 다시 저장하면 기존 항목을 갱신하며, 예전 명령 즐겨찾기는 한 번만 이전하고 원본 데이터는 보존합니다. 외부 명령 즐겨찾기는 공유 JSON과 작업 공간 자동 복원에서 제외됩니다. 호스트 카드의 빈 별은 즐겨찾기에 추가하고 채운 별은 해제합니다. 최근 호스트 접속 기록은 Hosts에 남아 있습니다.

Home의 **한 줄 연결**은 열기에 성공하면 입력을 비웁니다. 여는 중에 새로 입력한 내용과 실행하지 못한 초안은 유지합니다. **한 줄 연결**에 `ssh worker1` 또는 `multipass connect master`처럼 평소 쓰는 로컬 연결 명령을 입력하고 실행합니다. Sutty는 Windows의 직접 실행 파일을 새 로컬 터미널에 열므로 `ssh worker1`은 현재 PC의 `.ssh/config`, `IdentityFile`, Windows SSH Agent와 `PATH` 설정을 그대로 사용합니다. `multipass connect master`도 설치된 Multipass 실행 파일을 같은 방식으로 엽니다.
...
검증된 명령은 **★ 저장**으로 즐겨찾기에 저장하고, 즐겨찾기나 최근 기록을 한 번 눌러 다시 엽니다. 오른쪽 클릭 메뉴로 항목을 제거합니다. 즐겨찾기는 호스트 저장 한도 10,000개를 공유하고 최근 명령 기록은 최대 15개입니다. 모두 이 PC의 Sutty 데이터베이스에 저장됩니다. 해석된 실행 파일 경로와 터미널 출력은 저장하지 않고, 비밀번호·토큰·passphrase 형태의 인수는 저장을 거부합니다. 임의의 문자열이 비밀값인지 모두 식별할 수 없으므로 비밀값은 명령에 직접 넣지 마세요. 종료한 명령은 탭 재진입이나 앱 재시작으로 자동 실행되지 않습니다. 이 탭은 로컬 프로그램의 터미널이며 SFTP 파일 패널은 별도 SSH 연결을 사용합니다. 실제 SSH Agent·키 파일·Multipass·UI 상호작용 검증은 아직 완료되지 않았습니다.
...
4. **서버에 반영**에서 호스트·환경·원격 경로를 확인합니다. 원격 크기·수정시각·SHA-256 내용이 기준본과 다르면 자동 반영을 중지하고 덮어쓰기, 다른 절대 경로로 저장, 다시 내려받기를 검토합니다. 읽기·검증 실패도 자동 반영을 중지합니다. 다른 이름은 기존 파일을 덮어쓰지 않습니다.
...
업로드마다 별도 고정 복사본을 만들어 편집기의 다음 저장이 진행 중인 전송을 바꾸지 않게 합니다. 8 MiB 한도 내에서 SFTP로 원격 내용을 읽어 기준본의 SHA-256과 비교하므로 같은 크기·시각의 변경도 검출합니다. 읽기나 검증 실패 시 자동 반영을 중지합니다. 서버 측 잠금은 아니므로 비교 직후 다른 사용자가 수정하는 경쟁까지 방지하지 못합니다.
...
Hosts/History의 **호스트·명령 공유 / 가져오기** 진입 버튼은 제거했습니다. 공유 저장·미리보기 코드는 유지하지만 이 화면에서 제공하지 않습니다. 기존 호스트 형식 가져오기는 설정에서 사용할 수 있습니다. 카드 오른쪽 위 X는 저장 호스트 또는 해당 최근 접속 기록 한 건을 확인 후 삭제합니다. 최근 기록 삭제는 저장 호스트·즐겨찾기·다른 기록을 유지합니다.
...
After a working shell logs out or disconnects, its tab closes automatically by default. Disable automatic close in Settings to retain the ended tab. Automatic last-tab closure returns to Home without a replacement shell; initial failures remain visible and pending transfers or edits retain close confirmation. This follows actual PTY/process/connection state, never typed logout text.

A fresh start opens PowerShell in the center. Use `+` or `Ctrl+T` to add **Local PowerShell** or **Local CMD** tabs. Manually closing the last tab opens a replacement PowerShell tab. Optional workspace restoration preserves each local tab's shell choice; older entries without a shell value restore PowerShell.
...
Open **Multi Command** from the left navigation or `Alt+8`. It covers the central shell with a **3×3 session grid** while open shells keep running. Each occupied cell represents one open SSH, PowerShell, or CMD session. The first nine of up to 16 open tabs are displayed. Previous/next controls are hidden; extra tabs are counted as excluded and cannot become broadcast targets. Narrow windows retain three columns with scrolling.

The right side reuses the existing **command library**, including search, add/delete, and `$1`, `$2` parameter inputs. Both free-form commands entered above it and saved commands run **only on checked sessions**. Sutty SSH uses structured exec. Local/external targets require separate, default-off advanced input approval for every run. Input goes to the foreground program and may reach an editor or authentication prompt; completion without an exit status is labeled unverified. Review the exact command, all visible targets before approval; cancellation preserves the draft. SSH exec does not share the terminal directory, environment, or sudo state. A 60-second wait limit and cancel-wait action keep unknown remote outcomes explicit. **Select all** checks running local/external and connected Sutty SSH among the visible cards; **Clear all** removes selection. Hidden and disconnected tabs are excluded with visible counts. New sessions always start unchecked, with zero targets selected initially. Shell-tab and navigation changes preserve selection, last results, and running state for the same open sessions. Each cell shows session state and the last result, with a scrollable output preview of up to 16,384 characters. PROD-tagged targets require an additional confirmation.

Transfers and `Alt+7` follow the selected shell: the same SSH session's Local/Remote Files or the current directory reported by a PowerShell/CMD prompt. External tabs, missing shells, and unknown local paths retain an explicitly labeled local PC browser without choosing another server. Browsing locally does not change the shell directory; **Current shell folder** resumes following it once available. Directly launched SSH/Multipass programs are terminal-only: remote Files/Tunnels are not integrated. The connection button opens Home for a separate integrated SSH connection.

### One-line local connections

If you run `ssh B` inside server A's terminal, Files remains connected to A. To browse B, create a
new connection with B as the destination and choose **Advanced connection options → Connection
route → SSH Jump**, supplying A as the relay. Terminal and Files then use B. The relay must allow
TCP forwarding and reach B, and B must support SFTP. Supply destination authentication usable from
this PC; a key present only on A is not automatically reused. Files shows its actual SFTP endpoint.

A successful Open clears the submitted one-line input. A newer draft typed while opening and a rejected launch remain intact. On Home, enter a usual local connection command such as `ssh worker1` or `multipass connect master` in **One-line connection**. Sutty opens the direct Windows executable in a new local terminal, so `ssh worker1` continues to use the current PC's `.ssh/config`, `IdentityFile`, Windows SSH Agent, and `PATH`; Multipass uses its installed executable the same way. The launcher sends one executable plus arguments directly, never through `cmd.exe` or PowerShell. Shell chaining/redirection, executable paths, and command interpreters are rejected. SSH shortcuts are interactive host or configuration aliases only; they cannot include a remote command.

Use **★ Save** to keep a validated command, then select a favorite or recent entry to reopen it. Right-click an entry to remove it. Saved commands appear in Hosts favorites under the shared 10,000-profile limit; recent commands are bounded to 15 entries. Both remain local to this PC. Legacy favorites migrate once with original rows retained, and repeated saves update the same command. External favorites are excluded from sharing JSON and automatic workspace restoration. Empty stars add host favorites; filled stars remove them. Recent host connections remain in Hosts. The one-line connection card now sits separately below New connection on Home. Sutty stores neither the resolved executable path nor terminal output, and rejects common credential-shaped arguments from storage. Arbitrary positional secrets cannot all be recognized; keep secrets out of command text. Finished commands are never replayed by switching tabs or restarting the app. These tabs host local programs; SFTP uses a separate SSH connection. Live SSH Agent/key/config, Multipass, and UI-interaction validation remains unverified.

This development build adds folder navigation history, sorting, hidden-file controls, per-host remote bookmarks and copy-only pane-to-pane drag and drop. Remote text editing uses a local executable, explicit upload by default, an optional per-file save-upload switch, immutable upload copies, size/time and bounded remote SHA-256 content conflict checks and the existing verified transfer queue. Files remain recoverable under `%LOCALAPPDATA%\sutty\edits`; close the external editor and remove sensitive copies yourself when finished. Generic queue retry cannot bypass edit review after failure/restart. Edited files are limited to 8 MiB, with eight open copies per session.

Files → Terminal prepares/copies a quoted command without injecting or executing it. Open file path navigates the same session's Files explicitly. Tunnels provides session-local add/start/stop and real listener/error states with loopback defaults and explicit external-bind consent. Host menus provide duplication and authentication aliases; Settings retains host import. The Hosts/History sharing/import button is removed; sharing storage/preview code remains in the source. Each host/history card has an upper-right X with confirmation. Deleting a recent record preserves saved hosts, favorites, and other records. Export omits credentials, local key paths, trust records and history. External ProxyCommand profiles must be configured manually on the receiving PC.
