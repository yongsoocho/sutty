...
오른쪽은 기존 **명령 모음집**을 재사용하므로 검색·추가·삭제·`$1`, `$2` 값 입력을 그대로 사용할 수 있습니다. 위쪽에 직접 입력한 명령과 저장 명령 모두 **체크한 연결된 Sutty SSH 세션에만** 실행합니다. 로컬·외부 탭은 실행 대상에서 제외합니다. 실행 전 전체 대상 수·정확한 대상과 명령을 확인하며 취소해도 초안을 유지합니다. SSH exec는 현재 터미널의 cd·환경변수·sudo 상태를 공유하지 않습니다. 60초 대기 제한과 대기 취소를 제공하며 원격 작업의 종료를 보장하지 않습니다. **전체 선택**은 보이는 연결된 Sutty SSH만 선택하며 **전체 해제**는 선택 상태를 지웁니다. 숨겨진 탭과 로컬·외부·미연결 탭은 실행 대상에서 제외하고 개수를 표시합니다. 새로 연 세션은 항상 미선택이고 기본 대상은 0개입니다. 셸 탭·메뉴를 바꾸어도 같은 열린 세션의 선택·마지막 결과·진행 상태를 유지합니다. 각 칸에서 세션 상태와 마지막 결과를 확인하고 최대 16,384자의 출력 미리보기를 스크롤해 읽을 수 있습니다. PROD 태그가 있는 대상은 추가 확인을 거칩니다.

## 접속과 파일 탐색

Transfers나 `Alt+7`의 파일 탐색은 현재 선택한 셸을 따릅니다. SSH에서는 같은 세션의 Local/Remote Files를, PowerShell/CMD에서는 프롬프트가 알린 현재 작업 폴더를 표시합니다. 다른 서버를 대신 선택하지 않으며 셸이나 경로를 확인할 수 없으면 안내 상태를 표시합니다. 로컬 탐색기의 폴더 이동은 셸의 디렉터리를 바꾸지 않고 **현재 셸 폴더**로 다시 따라갈 수 있습니다. 직접 실행한 SSH/Multipass는 터미널 전용이며 원격 Files·Tunnels를 통합하지 않습니다. 호스트 가져오기 또는 저장 호스트 연결로 통합 도구를 사용하세요.
...
The right side reuses the existing **command library**, including search, add/delete, and `$1`, `$2` parameter inputs. Both free-form commands entered above it and saved commands run **only on checked connected Sutty SSH sessions**. Local/external tabs are excluded. Review the exact command, all visible targets before approval; cancellation preserves the draft. SSH exec does not share the terminal directory, environment, or sudo state. A 60-second wait limit and cancel-wait action keep unknown remote outcomes explicit. **Select all** checks connected Sutty SSH among the visible cards; **Clear all** removes selection. Hidden, local/external, and disconnected tabs are excluded with visible counts. New sessions always start unchecked, with zero targets selected initially. Shell-tab and navigation changes preserve selection, last results, and running state for the same open sessions. Each cell shows session state and the last result, with a scrollable output preview of up to 16,384 characters. PROD-tagged targets require an additional confirmation.

Transfers and `Alt+7` follow the selected shell: the same SSH session's Local/Remote Files or the current directory reported by a PowerShell/CMD prompt. Missing shells or unknown paths show an explanation without choosing another server. Browsing locally does not change the shell directory; **Current shell folder** resumes following it. Directly launched SSH/Multipass programs are terminal-only: remote Files/Tunnels are not integrated. Import hosts or connect a saved host for integrated tools.
