...
| Connection Doctor | Nine ordered stages produce bounded structured events, stable locale-independent error codes, user actions, safe copy summaries, and technical details without exception messages. Connection history retains the actual Password, PublicKey, Agent, or KeyboardInteractive type and the stable failure code. |
...
| Redacted support bundle | A user-created ZIP has an exact two-file allowlist (`manifest.json`, `report.json`), bounded correlation-specific stage events, deterministic archive layout, report size/hash binding, and atomic writes. It serializes no endpoint, username, path, host-key fingerprint, transcript, command, output, secret, or raw log field. |
