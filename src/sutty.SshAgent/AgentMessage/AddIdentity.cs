    ...
    internal class AddIdentity : IAgentMessage
    {
        ...
        private static void WriteKey(AgentWriter keyWriter, Key key)
        {
            ...
            switch (key.ToString())
            {
                ...
                case "ssh-rsa":
                    var rsa = (RsaKey)key;
                    keyWriter.EncodeBignum2(rsa.Modulus.ToByteArray().ReverseInPlace());
                    keyWriter.EncodeBignum2(rsa.Exponent.ToByteArray().ReverseInPlace());
                    keyWriter.EncodeBignum2(rsa.D.ToByteArray().ReverseInPlace());
                    keyWriter.EncodeBignum2(rsa.InverseQ.ToByteArray().ReverseInPlace());
                    keyWriter.EncodeBignum2(rsa.P.ToByteArray().ReverseInPlace());
                    keyWriter.EncodeBignum2(rsa.Q.ToByteArray().ReverseInPlace());
                    break;
                ...
                case "ecdsa-sha2-nistp521":
                    ...
                    keyWriter.EncodeString(publicKey[0].ToByteArray().ReverseInPlace());
                    keyWriter.EncodeString(publicKey[1].ToByteArray().ReverseInPlace());
                    // a key loaded from a private key file always has the scalar
                    keyWriter.EncodeBignum2(ecdsa.PrivateKey!.ToBigInteger2().ToByteArray().ReverseInPlace());
                    break;
                default:
