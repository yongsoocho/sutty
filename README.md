...
and **Tunnels** beside or, in narrow layouts, below it for that exact host. Cards, toolbars, and
file panes adapt to available width. **Commands** is the visible product label
for structured, non-interactive command cells; the internal persisted value `Repl` remains for
...
- The top `+` button and `Ctrl+T` open a menu for **New SSH connection**, **Open saved host**, **Local PowerShell**, **Local CMD**, and **Import hosts**; New SSH connection is emphasized by default. PowerShell and Windows Command Prompt (`cmd.exe`) use real ConPTY processes with runtime resize and process-tree cleanup.
...
- Up to 16 mixed local/SSH tabs and a separate **Multi Command** destination (`Alt+8`). A fixed 3×3 grid shows the first nine tabs, with horizontal scrolling when narrow. Pagination controls are hidden while their implementation is retained. Select all selects running local/external and connected Sutty SSH among the visible cards; Clear all removes selection. Local/external targets require a separate, default-off advanced terminal-input approval for every run. Counts explain disconnected and hidden tabs. Broadcast targets never include hidden tabs. New sessions start unchecked; exact-target/command and additional PROD confirmation remain required.
...
- `workspace.json` — up to 16 local-tab shell choices and opaque Saved Host ids; no credentials or terminal commands.
...
해당 Host의 옆 패널이나 좁은 화면의 아래 패널에 엽니다. 카드·도구 모음·파일 패널도
사용 가능한 너비에 맞춥니다. 구조화된 비대화형 명령 셀의 화면 이름은 **Commands**이며,
...
- 상단 `+` 버튼과 `Ctrl+T`는 **New SSH connection**, **Open saved host**, **Local PowerShell**, **Local CMD**, **Import hosts** 메뉴를 열며 New SSH connection을 기본으로 강조합니다. PowerShell과 Windows 명령 프롬프트(`cmd.exe`)는 실제 ConPTY 프로세스, 실행 중 크기 변경, 프로세스 트리 정리를 사용합니다.
...
- 로컬/SSH 혼합 최대 16개 탭과 독립 **Multi Command** 화면(`Alt+8`). 고정 3×3 그리드에 첫 9개 탭을 표시하며 좁으면 가로로 스크롤합니다. 페이지 이동 UI는 숨기고 내부 구현을 유지합니다. 전체 선택은 보이는 실행 중인 로컬·외부 탭과 연결된 Sutty SSH를 선택하며 전체 해제는 선택을 지웁니다. 로컬·외부 대상은 매 실행마다 기본 해제된 고급 터미널 입력 승인이 필요합니다. 미연결 탭과 숨겨진 탭은 제외 수를 표시하고 실행 대상에서도 제외합니다. 새 세션은 미선택이며 대상·명령 확인과 PROD 추가 확인은 유지합니다.
...
- 숫자 설정은 증감 버튼 없이 직접 입력합니다. About의 빨간 설정·SQLite 초기화는 각각 기본 선택 No인 Yes/No 확인 뒤 실행합니다. 설정은 기본값을 원자적으로 적용하며 SQLite는 트랜잭션으로 사용자 행만 삭제하고 스키마·마이그레이션 정보·열린 세션·별도 설정·키 신뢰·금고·복구 파일을 보존합니다.
- Transfers는 선택한 SSH의 Files 또는 PowerShell/CMD 프롬프트가 알린 현재 폴더를 전송 큐 위에 표시합니다. 다른 서버를 임의로 선택하지 않고 외부 탭이나 선택한 셸이 없으면 이 PC의 로컬 파일 목록과 별도 SSH 연결 안내를 표시합니다. 셸 탭을 바꿔도 선택한 보조 패널을 유지합니다.
- `CONPTY` / `PTY` 크기 표시 옆의 클립보드 아이콘으로 오류를 포함한 마지막 명령의 표시 출력을 복사합니다. PowerShell/CMD는 프로필 파일을 바꾸지 않는 실행 중 프롬프트 표시자를 사용합니다. 복사 대상은 원본 stdout/stderr 바이트가 아닌 터미널 텍스트이므로 PTY에서 탭·제어 문자·줄바꿈이 처리된 결과입니다. 사용자 정의 또는 표시자가 없는 SSH 셸의 명령 경계는 최선 방식으로 판별합니다.
...
- 키보드 중심 이동: `Alt+1` Home, `Alt+2` Hosts, `Alt+3` Transfers, `Alt+4` Commands, `Alt+5` Settings, `Alt+6` 선택 세션의 Terminal, `Alt+7` 해당 세션의 Files, `Alt+8` Multi Command. Files는 PowerShell/CMD 탭의 로컬 탐색기도 엽니다. `Ctrl+1`–`Ctrl+9`는 탭을 전환하고 `Ctrl+T`는 `+` 메뉴를 열며, `Ctrl+,`는 Settings를 엽니다. Insert 방식 복사·붙여넣기도 유지합니다. Alt 화면 전환은 시스템 메뉴 처리 전에 등록·소비되며, 정확한 배포 후보에서 기계음이 나지 않는지는 수동 릴리스 검사로 남아 있습니다. [키보드 단축키](docs/KEYBOARD_SHORTCUTS.md)를 참고하세요.

### GA가 아닌 이유

- 2026-09-22 배치·테마·복사 변경의 Windows 화면 자동화는 **사용자의 Escape 입력으로 중단**되었습니다. 화면과 수동 클립보드 인수 검증은 미완료입니다.
...
- `workspace.json` — 최대 16개의 로컬 탭 셸 선택과 불투명 저장 Host ID. 자격증명과 터미널 명령은 기록하지 않음
