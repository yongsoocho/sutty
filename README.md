...
> **Alpha, not GA.** Sutty is an active engineering build. Do not treat it as a complete PuTTY, FileZilla, Termius, or MobaXterm replacement, and review the limitations before using important systems.

> **알파, GA 아님.** Sutty는 개발 중인 엔지니어링 빌드입니다. 아직 PuTTY, FileZilla, Termius, MobaXterm의 완전한 대체 제품으로 취급하면 안 되며, 중요한 시스템에 사용하기 전에 한계를 확인해야 합니다.

## English

Sutty aims to replace the everyday combination of **PuTTY for SSH**, **FileZilla for SFTP**, and selected **Windows-local productivity workflows from Termius and MobaXterm**. That is a product goal, not a statement of current feature parity.
...
- SSH agent, OTP/multi-prompt keyboard-interactive UI, PuTTY `.ppk`, jump hosts, proxies, and automatic reconnect are unavailable. Password mode's non-interactive fallback handles password-like prompts only. Keepalive is available; disabled controls are not capabilities.
...
- SFTP currently transfers files, not directory trees. It has no pause/retry/resume, final size/checksum verification, `chmod`, synchronized browsing, or complete FileZilla collision policies.
...
Sutty는 **PuTTY의 SSH**, **FileZilla의 SFTP**, 그리고 **Termius와 MobaXterm의 Windows 로컬 생산성 작업 흐름**을 일상 서버 작업에서 대체하는 것을 목표로 합니다. 이는 제품 목표이며 현재 기능이 모두 동등하다는 주장이 아닙니다.
...
- SSH agent, OTP·다중 prompt keyboard-interactive UI, PuTTY `.ppk`, 점프 호스트, 프록시, 자동 재연결은 지원하지 않습니다. 비밀번호 방식의 비대화형 fallback은 password 형태 prompt만 처리합니다. Keepalive는 사용할 수 있지만 비활성화된 컨트롤은 기능이 아닙니다.
- History와 pin은 완전한 Saved Host Profile/Vault가 아닙니다. 암호화된 자격 증명 Vault가 없으므로 비밀정보는 다시 입력해야 합니다.
- 현재 SFTP는 파일만 전송하며 디렉터리 트리는 전송하지 않습니다. 일시정지·재시도·재개, 최종 크기/checksum 검증, `chmod`, 동기 탐색, FileZilla 수준의 전체 충돌 정책이 없습니다.
