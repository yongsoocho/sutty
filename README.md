...
- Dual-pane Files with a read-only local Details list on the left and the active session's remote Details list on the right. Both panes support absolute-path entry, parent navigation, refresh, and extended multi-selection; explicit buttons queue file/folder transfers in either direction through the existing collision, staging, checkpoint, and verification policies. Remote filename search, rename, cross-directory move without overwrite, safe recursive deletion, octal permission changes, directory creation, Copy path, and Open in Terminal remain available. Pane-to-pane drag-and-drop, synchronized browsing, directory comparison, sorting controls, and favorite paths are not yet implemented; Windows Explorer → Remote drop remains available.
...
- 왼쪽의 읽기 전용 로컬 Details 목록과 오른쪽의 활성 세션 원격 Details 목록으로 구성한 Dual-pane Files. 양쪽 모두 절대 경로 입력·상위 폴더·새로 고침·다중 선택을 지원하며, 명시적 버튼으로 기존 충돌 정책·안전 staging·checkpoint·검증 경로를 거쳐 파일과 폴더를 양방향 전송합니다. 원격 파일명 검색, 이름 변경, 덮어쓰기 없는 디렉터리 간 이동, 안전한 재귀 삭제, 8진수 권한 변경, 디렉터리 생성, 경로 복사, Terminal에서 열기도 유지합니다. Pane 간 drag-and-drop, 동기 탐색, 디렉터리 비교, 정렬 컨트롤, 즐겨찾는 경로는 아직 구현하지 않았고 Windows Explorer → Remote drop은 유지합니다.
